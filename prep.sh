...
for gamma in 0.0 #0.001 0.01 0.1 #0.0 0.1 0.5 #1.0 2.0 #0.00001
do
...
if [ $method == "igg-cn" ]; then
    echo "Method: " $method
    code_dir=igg-cn
    cd ${master_dir}/runs/many-bubbles-test
    run_dir=${method}-gamma-${gamma}-bubbles-${no_bubbles}
    mkdir $run_dir
    cp ${master_dir}/${code_dir}/* ${master_dir}/runs/many-bubbles-test/${run_dir}/
    cd ${master_dir}/runs/many-bubbles-test/${run_dir}
