...
tol = prec(1e-4)
#Pheno damping term
γ_inp=parse(prec,ARGS[1])
#Temperature-going with 1/4\eta for now
T=parse(prec,ARGS[9])

export β_Y,β_W
β_Y = parse(prec,ARGS[10])
β_W = parse(prec,ARGS[11])
...
base_seed = 123456*parse(int_prec,ARGS[12]) ##Need to change up the seed just in case any 2 or more runs launch at the same time.
